...
IF ~~ THEN REPLY ~Yes, any help you can give would be welcomed.~ DO ~GiveItemCreate("POTN21",Player1,6,0,0)~ DO ~GiveItemCreate("KTSnow",Player1,1,0,0)~
EXIT
END

IF ~Global("KTQuest","AR2300",5)~ THEN BEGIN KTMonk4
SAY ~Bag of Red I sense with you! Toils and troubles you have seen, a gift from we three to bestow upon thee. Two there were but lost one did I, to Chaos I assume ... but seek each other they will. Sense to you it will make, as time and troubles pass. For now we trade our gifts to one another~
IF ~~ THEN REPLY ~Yes ... here we have your Red Bag.~ DO ~TakePartyItem("KTSnow")~ DO ~TakePartyItem("KTRedB")~ DO ~GiveItemCreate("KTHlm01",Player1,0,0,0)~ DO ~SetNumTimesTalkedTo(4)~ DO ~SetGlobal("KTQuest","AR2300",6)~ EXIT
END

IF ~NumTimesTalkedTo(4)~THEN BEGIN KTMonk5
SAY ~Promises made, promises kept ... now see gifts from afar?~
IF ~~ THEN REPLY ~Yes, what can you show me today?~ DO ~StartStore("KTStore",LastTalkedToBy(Myself))~ DO ~SetNumTimesTalkedTo(4)~ EXIT
IF ~~ THEN REPLY ~No, not right now.~ DO ~SetNumTimesTalkedTo(4)~ EXIT
END
END